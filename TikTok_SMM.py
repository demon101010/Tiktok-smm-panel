...
class Zefoy:
    ...
    def get_captcha(self):
        ...
        if 'Video URL' in request.text: self.video_key = request.text.split('" placeholder="Video URL"')[0].split('name="')[-1]; return True
        ...
        try:
            ...
            print(f"- Mk Cenabeti Eksiksiz Yükle Pip'leri: {e}")
            time.sleep(2)
            self.get_captcha()

    def send_captcha(self, new_session = False):
        if new_session: self.session = requests.Session(); os.remove('session'); time.sleep(2)
        if self.get_captcha(): print('- Yükleniyor Reis Bekle .... ');return (True, 'Oturum zaten mevcut')
        ...
        if 'Video URL' in request.text: 
            print('Oturum oluşturuldu')
            open('session','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))
            print(f"Captcha başarıyla çözüldü: {captcha_solve}")
            self.video_key = request.text.split('" placeholder="Video URL"')[0].split('name="')[-1]
    ...
    def get_table(self, i = 1):
        ...
        while True:
            ...
            else:print('Hizmetler alınamıyor, yeniden deneniyor...');self.send_captcha();time.sleep(2)
        os.system('clear')
        print(output)
        print("~ Developer : @PythonCodersPro | Channel: @endlessarchivist ~")
        print("\x1b[1;33m—" * 60)
        for service in self.services:
            ...
            i += 1
        table = f"\n{Fore.GREEN}- Tüm Aktif Servisler => {len([x for x in self.services_status if self.services_status[x]])}{Fore.RESET}"
        print(table)
        print("\x1b[1;33m—" * 60)
    def find_video(self):
        if self.service is None: return (False, "Hizmeti seçmediniz")
        while True:
            ...
            if 'Oturum süresi doldu. Lütfen yeniden giriş yapın' in self.video_info:
                print('Oturum süresi doldu. Tekrar giriş yapıyorum...')
                self.send_captcha()
                return
            elif 'hizmet şu anda çalışmıyor' in self.video_info:
                return (True, 'Hizmet şu anda kullanılamıyor, lütfen daha sonra tekrar deneyin.')
            ...
            elif 'Zamanlayıcıyı Kontrol Etme...' in self.video_info:
                try: 
                ...
                while time.time()-2<_+zyfoy:
                    t-=1
                    print("- \033[1;32mSahmeran \033[1;33mTools • \033[1;31mBekle:\033[1;36m {0} ".format(t)+"\033[1;35msecond\033[1;32m .", end="\r")
                    time.sleep(1)
                continue
            elif 'Çok fazla istek. Lütfen yavaş' in self.video_info:
                time.sleep(3)
            else:
    ...
    def use_service(self):
        ...
        try:
            ...
            time.sleep(3)
            return ""
        if 'Oturum süresi doldu. Lütfen yeniden giriş yapın' in res:
            print('Oturum süresi doldu. Tekrar giriş yapıyorum...')
            self.send_captcha()
            return ""
        elif 'Çok fazla istek. Lütfen yavaş' in res:
            time.sleep(3)
        elif 'hizmet şu anda çalışmıyor' in res:
            ...
            print(f'{a3} \n—————————————————————————————\n {ble}- {a4}Name {a1}=> {ble}{name}\n{ble} - {a4}UserName {a1}=> {ble}@{username}\n{ble} - {a4}Takipçi {a1}=> {ble}{followers}\n {ble}- {a4}Takipçi {a1}=> {ble}{following}\n {ble}- {a4}Videos {a1}=> {ble}{videos}\n {ble}- {a4}Video İzlenme {a1}=> {ble}{views}\n{ble} - {a4}Video Beğeni {a1}=> {ble}{likes}\n {ble}- {a4}Video Paylaş {a1}=> {ble}{shares}\n {ble}- {a4}Video Comments {a1}=> {ble}{comments}\n{a3}—————————————————————————————\n')
    ...
    def get_video_id(self, url_ = None, set_url=True):
        ...
        if request.text == '':
            print('Video bağlantısı geçersiz')
            return False
        else:
            json_ = request.json()
        if 'author' not in json_:
            print(f'{self.url}|  Video bağlantısı geçersiz')
            return False
        if set_url:
            self.url = f'https://www.tiktok.com/@{json_["author"]}/video/{json_["id"]}'
            print(f'Örnek video URLsi --> {self.url}')
        return request.text
    ...
    def update_name(self):
        while True:
            try:
                ...
                self.text = f"İzlenme : {video_info['viewCount']} "
            except:
                pass
            time.sleep(5)
    ...
    def select_service(self):
        while True:
            ...
            self.get_table()
            print(f"{xanh_la}- Numara Seç ;", end=' ')
            service_id = input()
            url1=input('-{}TikTok{} Video {}Link : {}'.format(gre,ble,red,yel,gre))
            self.url=url1
            if service_id.isdigit():
                ...
                if service_id in range(1, len(self.services) + 1):
                    ...
                    print(f"{do}Basitçe Sayı Girmek Yanlıştır.")
            else:
                print(f"{do}Basitçe Sayı Girmek Yanlıştır.")
    ...
    def run(self):
        ...
        while True:
            ...
            tim = "\033[1;35m"
            try:
                if 'Hizmet şu anda çalışmıyor, daha sonra tekrar deneyin' in str(self.use_service()):
                    print(f'{do}[\033[1;33mHizmet şu anda kullanılamıyor, lütfen daha sonra tekrar deneyin.')
                    time.sleep(5)
            except Exception as e:
                print(f'CİDDİ HATA | 10 saniye sonra tekrar deneyin.|| {e}');time.sleep(10)
                time.sleep(0)
